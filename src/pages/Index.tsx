...
export default function Index() {
  ...
  const getBookingText = () => {
    return `🚖 Новая заявка — Такси АМГ\n\n📍 Откуда: ${form.from}\n🏁 Куда: ${form.to}\n📅 Дата: ${form.date}\n👤 Имя: ${form.name}\n📞 Телефон: ${form.phone}`;
  };

  const getTelegramLink = () => {
    return `https://t.me/taxi_dlnrRF?text=${encodeURIComponent(getBookingText())}`;
  };
          ...
          <div className="max-w-4xl mx-auto">
            ...
            {sent ? (
              <div className="card-glass rounded-2xl p-12 text-center">
                ...
                <h3 className="font-display text-3xl font-bold mb-3">Заявка готова!</h3>
                <p className="text-muted-foreground font-body text-lg mb-6">В Telegram данные уже вставлены в сообщение. Для MAX текст заявки скопирован — просто вставьте его в чат</p>
                <div className="flex flex-wrap justify-center gap-4">
                  ...
                  <a href={MAX_LINK} target="_blank" rel="noreferrer"
                    className="inline-flex items-center gap-2 border-2 border-orange-500 text-orange-500 font-display font-bold px-8 py-3 rounded-full hover:bg-orange-500/10 transition-all">
              ...
              <form onSubmit={handleSubmit} className="card-glass rounded-2xl p-8 md:p-10">
                ...
                <div className="flex flex-wrap gap-4">
                  ...
                  <a href={MAX_LINK} target="_blank" rel="noreferrer"
                    onClick={() => { navigator.clipboard?.writeText(getBookingText()); handleSubmit({ preventDefault: () => {} } as React.FormEvent); }}
                    className="inline-flex items-center gap-2 border-2 border-orange-500 text-orange-500 font-display font-bold px-10 py-4 rounded-full hover:bg-orange-500/10 transition-all text-lg cursor-pointer">
